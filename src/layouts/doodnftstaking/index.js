...
import "./doodnftstaking.css";

import doodCardIMG from "../../assets/images/doodCardIMG.png";
// Contract ABI
import DOODDNFTABI from "../../assets/abi/doodnftABI.json";
import SDOODABI from "../../assets/abi/sdoodABI.json";
import DOODNFTSTAKINGABI from "../../assets/abi/doodnftStakingABI.json";

import config from "../../config/config";

import NFTCard from "./doodnftcard";

const ethers = require("ethers");

function DoodNftStaking() {
  ...
  const usernftContract = new ethers.Contract(config.DOODNFTADDRESS, DOODDNFTABI, stakingSigner);
  ...
  const getStakeData = async () => {
    ...
    const approveState = localStorage.getItem("approve_token");
        ...
        await stakingContract.stakedNFTs(Number(data[i].toString())).then((stakeInfo) => {
          stakingContract.calculateRewardsNFT(Number(data[i])).then((totalreward) => {
            ...
            stakedNFTArray.push({
              ...
              src: `${config.baseURI}/${data[i].toString()}.png`,
              isStaked: true,
            });
          });
        });
  ...
  const approveAllFunc = async () => {
    await usernftContract.setApprovalForAll(config.SDOODSTAKINGADDRESS, true).then((tx) => {
      tx.wait().then(() => {
    ...
    <DashboardLayout>
      ...
      <MDBox pt={6} pb={3}>
        <Grid container spacing={6}>
          <Grid item xs={12}>
            <Card className="doodnftstakingContainer">
              <MDBox
                mx={2}
                mt={-3}
                py={3}
              ...
              <MDBox pt={3} style={{ minHeight: "500px" }}>
                <Grid container spacing={8} p={3}>
                  ...
                  >
                    <img src={doodCardIMG} className="doodCardIMG" alt="doodCardIMG" />
                  </Grid>
