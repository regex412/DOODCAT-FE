...
function NFTCard({ tokenID, isStaked, balance, src, level }) {
  ...
  const stackFunc = async () => {
    ...
    await stakingContract.stake(nftIDArray, { gasLimit: 3000000 }).then((tx) => {
      tx.wait().then(() => {
  ...
  const unstakeFunc = async () => {
    ...
    const nftIDArray = [tokenID];
    await stakingContract.unStake(nftIDArray, { gasLimit: 3000000 }).then((tx) => {
      tx.wait().then(() => {
        notification.success({
          message: "Success",
  ...
  const upgrade = async () => {
    ...
    if (Number(level) > 3) {
      ...
      if (amountValue > sdoodContract.balanceOf(sdoodBalance)) {
        ...
        await sdoodContract.approve(account, amountValue, { gasLimit: 3000000 }).then((tx) => {
          tx.wait().then(() => {
            sdoodContract
              .transferFrom(account, config.DOODNFTSTAKINGADDRESS, amountValue, {
                gasLimit: 3000000,
              })
              .then((tx1) => {
                tx1.wait().then(() => {
                  stakingContract.UpgradeLevel(tokenID, { gasLimit: 3500000 }).then((tx2) => {
                    tx2.wait().then(() => {
                      notification.success({
                        message: "Success",
        ...
        await sdoodContract.approve(account, amountValue, { gasLimit: 3000000 }).then((tx) => {
          tx.wait().then(() => {
            sdoodContract
              ...
              .then((tx1) => {
                tx1.wait().then(() => {
                  stakingContract.UpgradeLevelMax(tokenID, { gasLimit: 3500000 }).then((tx2) => {
                    tx2.wait().then(() => {
                      notification.success({
                        message: "Success",
